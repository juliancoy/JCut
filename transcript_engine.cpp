...
{
    m_transcriptSourceWordRangesCache.clear();
    m_transcriptWordRangesCache.clear();
...
void TranscriptEngine::setLiveTranscriptDocument(const QString& transcriptPath,
...
void TranscriptEngine::clearLiveTranscriptDocument(const QString& transcriptPath)
{
    if (transcriptPath.trimmed().isEmpty()) return;
    m_liveTranscriptDocuments.remove(QFileInfo(transcriptPath).absoluteFilePath());
    invalidateCache();
}
