                                                  ...
                                                  qreal boxSizeNorm)
{
    ...
    const bool mutated = session.mutateRoot([&](QJsonObject& root) {
        ...
        tracking[QString(referenceIndex == 1 ? kTranscriptSpeakerTrackingRef1Key
                                             : kTranscriptSpeakerTrackingRef2Key)] = refObj;
                                                  ...
                                                  const QString& speakerId)
{
    ...
    const bool mutated = session.mutateRoot([&](QJsonObject& root) {
        QJsonObject profiles = root.value(QString(kTranscriptSpeakerProfilesKey)).toObject();
        QJsonObject profile = profiles.value(speakerId).toObject();
        if (!profile.contains(QString(kTranscriptSpeakerFramingKey)) &&
            !profile.contains(QString(kTranscriptSpeakerTrackingKey))) {
            return false;
        }
