...
QJsonArray sectionTrackEntriesFromAssignment(const QJsonObject& assignment)
{
    ...
    QJsonObject entry{
        ...
        {QStringLiteral("rotation"), assignment.value(QStringLiteral("rotation")).toDouble(0.0)}
    };
...
int sectionWordCountFromTranscript(const QJsonObject& transcriptRoot,
                                   const QString& speakerId,
                                   int64_t startFrame,
                                   int64_t endFrame)
{
    ...
    for (const QJsonValue& segValue : segments) {
        ...
        if (segmentSpeaker != trimmedSpeakerId || segmentObj.value(QStringLiteral("skipped")).toBool(false)) {
            continue;
        }
...
int sectionAssignmentWordCount(const QJsonObject& transcriptRoot, const QJsonObject& row)
{
    const int stored = row.value(QStringLiteral("word_count")).toInt(-1);
    if (stored >= 0) {
        return stored;
    }
    return sectionWordCountFromTranscript(
        transcriptRoot,
        row.value(QStringLiteral("speaker_id")).toString().trimmed(),
        row.value(QStringLiteral("start_frame")).toVariant().toLongLong(),
        row.value(QStringLiteral("end_frame")).toVariant().toLongLong());
}
...
QPixmap SpeakersTab::speakerSectionTrackAvatarStrip(
...
{
    const int tileSize = qBound(24, avatarSize, 56);
    const int gap = 4;
    QVector<QPixmap> avatars;
    ...
    for (const QJsonValue& entryValue : assignedTrackEntries) {
        ...
        if (firstTrackId && *firstTrackId < 0) {
            *firstTrackId = trackId;
        }

        const QJsonObject entryAssignment = sectionTrackEntryAssignment(sectionAssignment, entry);
        QJsonObject avatarStream = streamByTrackId.value(trackId);
        ...
        QPixmap avatar;
        if (!avatarStream.isEmpty()) {
            avatar = continuityTrackAvatar(clip,
                                           speakerId,
                                           avatarStream,
                                           tileSize,
                                           decoderCtx,
        ...
        if (avatar.isNull()) {
            avatar = unsetSpeakerAvatar(tileSize);
        }

        QImage labeled(tileSize, tileSize, QImage::Format_ARGB32_Premultiplied);
        labeled.fill(Qt::transparent);
        QPainter tilePainter(&labeled);
        tilePainter.setRenderHint(QPainter::Antialiasing, true);
        tilePainter.drawPixmap(QRect(0, 0, tileSize, tileSize), avatar);
        const QRectF badgeRect(tileSize - 22.0, tileSize - 15.0, 20.0, 13.0);
        tilePainter.setPen(QPen(QColor(QStringLiteral("#071018")), 1.0));
        tilePainter.setBrush(QColor(QStringLiteral("#d9e9f8")));
        tilePainter.drawRoundedRect(badgeRect, 3.0, 3.0);
        QFont badgeFont = tilePainter.font();
        badgeFont.setBold(true);
        badgeFont.setPointSize(qMax(6, tileSize / 5));
        tilePainter.setFont(badgeFont);
        tilePainter.setPen(QColor(QStringLiteral("#071018")));
        tilePainter.drawText(badgeRect.toRect(), Qt::AlignCenter, QStringLiteral("T%1").arg(trackId));
        tilePainter.end();
        avatars.push_back(QPixmap::fromImage(labeled));
    }
    ...
    const int stripWidth = (avatars.size() * tileSize) + ((avatars.size() - 1) * gap);
    QImage strip(stripWidth, tileSize, QImage::Format_ARGB32_Premultiplied);
    strip.fill(Qt::transparent);
    QPainter painter(&strip);
    painter.setRenderHint(QPainter::Antialiasing, true);
    for (int i = 0; i < avatars.size(); ++i) {
        painter.drawPixmap(i * (tileSize + gap), 0, avatars.at(i));
    }
    painter.end();
...
void SpeakersTab::applyLoadedTranscriptDocumentData(const TimelineClip& clip, const QString& preferredSpeakerId)
{
    ...
    syncSpeakerSectionMinimumWordsControl(clip);
...
void SpeakersTab::onSpeakerCreateTitleClipsClicked()
{
    ...
    if (m_widgets.speakerOverlayRotationXSpin)
        flyInSettings.rotationStartXDegrees = m_widgets.speakerOverlayRotationXSpin->value();
    if (m_widgets.speakerOverlayRotationYSpin)
        flyInSettings.rotationStartYDegrees = m_widgets.speakerOverlayRotationYSpin->value();
    if (m_widgets.speakerOverlayRotationZSpin)
        flyInSettings.rotationStartZDegrees = m_widgets.speakerOverlayRotationZSpin->value();
    ...
    flyInSettings.showSpeakerOrganization =
        m_widgets.speakerShowCurrentSpeakerOrganizationCheckBox &&
        m_widgets.speakerShowCurrentSpeakerOrganizationCheckBox->isChecked();
    ...
    if (m_widgets.speakerOverlayTitleExtrudeModeCombo) {
        flyInSettings.titleExtrudeMode =
            static_cast<TimelineClip::TitleKeyframe::TextExtrudeMode>(
                m_widgets.speakerOverlayTitleExtrudeModeCombo->currentData().toInt());
    }
    ...
    QVector<TimelineClip> generatedTitles = makeSpeakerTitleClipsForTranscriptIntroductions(
        *clip, transcriptPath, sections, clip->trackIndex + 1, flyInSettings);
    const int appliedTitleCount = generatedTitles.size();
    const bool updated = m_speakerDeps.updateClipById(
        clipId,
        [&](TimelineClip& editableClip) {
            editableClip.titleKeyframes.clear(); // migrate legacy embedded speaker titles
            editableClip.speakerTitleEngineActive = !generatedTitles.isEmpty();
            editableClip.transcriptOverlay.showSpeakerTitle = false;
        });
    ...
    const GeneratedClipPlacementResult placementResult =
        m_speakerDeps.replaceSpeakerTitleClips(clipId, generatedTitles);
    if (m_speakerDeps.refreshPreview) {
        m_speakerDeps.refreshPreview();
    }
...
void SpeakersTab::syncSpeakerListMode()
{
    ...
    if (m_widgets.speakerSectionMinimumWordsSpin) {
        m_widgets.speakerSectionMinimumWordsSpin->setVisible(showSections);
    }
...
int SpeakersTab::speakerSectionMinimumWords() const
{
    if (m_widgets.speakerSectionMinimumWordsSpin) {
        return qBound(0, m_widgets.speakerSectionMinimumWordsSpin->value(), 1000);
    }
    const TimelineClip* clip = m_deps.getSelectedClip ? m_deps.getSelectedClip() : nullptr;
    return clip ? qBound(0, clip->speakerSectionMinimumWords, 1000) : 10;
}

void SpeakersTab::syncSpeakerSectionMinimumWordsControl(const TimelineClip& clip)
{
    if (!m_widgets.speakerSectionMinimumWordsSpin) {
        return;
    }
    QSignalBlocker blocker(m_widgets.speakerSectionMinimumWordsSpin);
    m_widgets.speakerSectionMinimumWordsSpin->setValue(
                                         ...
                                         int64_t sourceFrame)
{
    ...
    pushPreviewAssignedFaceTrackIdsForSpeakerAtFrame(*clip, activeSpeakerId, sourceFrame);
...
bool SpeakersTab::seekToSpeakerSectionRow(int row, int64_t* timelineFrameOut)
{
    if (!m_widgets.speakerSectionsTable || row < 0 || !m_deps.seekToTimelineFrame) {
        return false;
    }
    QTableWidgetItem* speakerItem =
        m_widgets.speakerSectionsTable->item(row, SpeakerSectionSpeakerColumn);
    ...
    const QVector<RenderSyncMarker> markers = m_speakerDeps.getRenderSyncMarkers
        ? m_speakerDeps.getRenderSyncMarkers()
        : QVector<RenderSyncMarker>{};
    const int64_t timelineFrame =
        timelineFrameForClipTranscriptFrame(*clip, transcriptFrame, markers);
    if (timelineFrameOut) {
        *timelineFrameOut = timelineFrame;
    }
    const int64_t currentFrame = m_deps.getCurrentTimelineFrame
        ? m_deps.getCurrentTimelineFrame()
        : -1;
    m_suppressSyncForTimelineFrame = timelineFrame;
                                                                    ...
                                                                    int64_t sourceFrame) const
{
    ...
    if (contiguousTranscriptSectionModeActive()) {
        ...
        const QJsonObject resolvedPayload =
            clipRoot.value(QStringLiteral("resolved_current")).toObject();
        const QJsonArray sectionMap =
            resolvedPayload.value(QStringLiteral("section_track_map")).toArray();
        for (const QJsonValue& value : sectionMap) {
            ...
            if (sectionAssignmentWordCount(m_transcriptSession.rootObject(), row) <
                qBound(0, clip.speakerSectionMinimumWords, 1000)) {
                continue;
            }
            for (const QJsonValue& entryValue : sectionTrackEntriesFromAssignment(row)) {
                const int trackId = entryValue.toObject().value(QStringLiteral("track_id")).toInt(-1);
                if (trackId >= 0) {
                    trackIds.insert(trackId);
                }
            }
    ...
    const QSet<int> speakerTrackIds =
        resolvedCurrentTrackIdsForSpeaker(m_transcriptSession.rootObject(), clip.id, trimmedSpeakerId);
    for (int trackId : speakerTrackIds) {
        if (trackId >= 0) {
            trackIds.insert(trackId);
        }
    }
...
void SpeakersTab::pushPreviewAssignedFaceTrackIdsForSpeakerAtFrame(const TimelineClip& clip,
                                                                   const QString& speakerId,
                                                                   int64_t sourceFrame)
{
    if (!m_speakerDeps.setPreviewAssignedFaceTrackIds) {
    ...
    m_speakerDeps.setPreviewAssignedFaceTrackIds(
        previewAssignedFaceTrackIdsForSpeakerAtFrame(clip, speakerId, sourceFrame));
}
...
void SpeakersTab::refreshSpeakerSectionsTable(const QJsonObject& transcriptRoot)
{
    ...
    auto finalizeRefreshTiming = [this, &refreshTimer]() {
        m_lastSpeakerSectionsTableRefreshDurationMs = refreshTimer.elapsed();
        m_maxSpeakerSectionsTableRefreshDurationMs =
            qMax(m_maxSpeakerSectionsTableRefreshDurationMs,
                 m_lastSpeakerSectionsTableRefreshDurationMs);
    };
    if (!m_widgets.speakerSectionsTable) {
        m_lastSpeakerSectionsTableRefreshSkippedReason = QStringLiteral("no_table");
        finalizeRefreshTiming();
        return;
    }
    if (!contiguousTranscriptSectionModeActive()) {
        m_lastSpeakerSectionsTableRefreshSkippedReason = QStringLiteral("sections_mode_inactive");
        finalizeRefreshTiming();
        return;
    }
    ...
    const QString refreshSignature =
        (clip ? clip->id : QStringLiteral("no_clip")) + QLatin1Char('|') +
    ...
    if (refreshSignature == m_speakerSectionsTableRefreshSignature &&
        m_widgets.speakerSectionsTable->rowCount() > 0) {
        if (!selectedSectionSpeakerId.isEmpty()) {
            ...
            for (int row = 0; row < m_widgets.speakerSectionsTable->rowCount(); ++row) {
                ...
                const int64_t rowStartFrame =
                    speakerItem->data(SpeakerSectionStartFrameRole).toLongLong();
                if (rowSpeakerId == selectedSectionSpeakerId &&
                    rowStartFrame == selectedSectionStartTimelineFrame) {
                    m_widgets.speakerSectionsTable->setCurrentCell(row, SpeakerSectionSpeakerColumn);
                    break;
                }
            }
        }
        ++m_speakerSectionsTableRefreshCacheHitCount;
        m_lastSpeakerSectionsTableRefreshSkippedReason = QStringLiteral("cache_hit");
        finalizeRefreshTiming();
        return;
    }
    ...
    for (int sectionIndex = 0; sectionIndex < rows.size(); ++sectionIndex) {
        ...
        const qreal sectionRotation =
            qBound<qreal>(-180.0, sectionAssignment.value(QStringLiteral("rotation")).toDouble(0.0), 180.0);
        QStringList trackLabels;
        int avatarTrackId = -1;
        const QPixmap avatarStrip =
            clip ? speakerSectionTrackAvatarStrip(*clip,
                                                  section.speakerId,
                                                  sectionAssignment,
                                                  assignedTrackEntries,
                                                  streamByTrackId,
                                                  40,
                                                  avatarDecoder.get(),
        ...
        avatarItem->setSizeHint(QSize(qMax(40, avatarStrip.width()), 40));
        if (!avatarStrip.isNull()) {
            avatarIconWidth = qMax(avatarIconWidth, avatarStrip.width());
            avatarItem->setIcon(QIcon(avatarStrip));
            avatarItem->setToolTip(QStringLiteral("Continuity-track avatar(s): %1")
        ...
        auto* rotationItem = new QTableWidgetItem(QString::number(sectionRotation, 'f', 1));
        rotationItem->setTextAlignment(Qt::AlignRight | Qt::AlignVCenter);
        rotationItem->setToolTip(QStringLiteral("Section rotation in degrees around the mapped face box."));
    ...
    m_lastSpeakerSectionsTableRowCount = rows.size();
    m_speakerSectionsTableRefreshSignature = refreshSignature;
                                                          ...
                                                          int64_t onlyEndFrame)
{
    ...
    for (int row = 0; row < m_widgets.speakerSectionsTable->rowCount(); ++row) {
        ...
        QStringList trackLabels;
        int avatarTrackId = -1;
        const QPixmap avatarStrip = speakerSectionTrackAvatarStrip(*clip,
                                                                   trimmedSpeakerId,
                                                                   sectionAssignment,
                                                                   assignedTrackEntries,
                                                                   streamByTrackId,
        ...
        if (QTableWidgetItem* avatarItem =
            ...
            avatarItem->setToolTip(
                trackLabels.isEmpty()
                    ? QString()
                    : QStringLiteral("Continuity-track avatar(s): %1")
                          .arg(trackLabels.join(QStringLiteral(", "))));
            avatarIconWidth = qMax(avatarIconWidth, avatarStrip.width());
        }
        ...
        if (QTableWidgetItem* rotationItem =
                m_widgets.speakerSectionsTable->item(row, SpeakerSectionRotationColumn)) {
            ...
            rotationItem->setData(SpeakerSectionRotationRole, sectionRotation);
            rotationItem->setText(QString::number(sectionRotation, 'f', 1));
            rotationItem->setToolTip(QStringLiteral("Section rotation in degrees around the mapped face box."));
        }
...
void SpeakersTab::updateSpeakerTrackingStatusLabel()
{
    ...
    if (m_widgets.speakerSectionRotationSpin) {
        m_widgets.speakerSectionRotationSpin->setEnabled(
            canEditClipFraming &&
            contiguousTranscriptSectionModeActive() &&
            m_widgets.speakerSectionsTable &&
            m_widgets.speakerSectionsTable->currentRow() >= 0);
    }
    ...
    QString tooltip =
        QStringLiteral("UI timings ms | Speakers: %1 (max %2) | Sections: %3 (max %4, rows %5, %6) | Playhead: %7 (max %8) | Tracks: %9 (max %10) | Detections: %11 (max %12)")
            .arg(m_lastSpeakersTableRefreshDurationMs)
            .arg(m_maxSpeakersTableRefreshDurationMs)
            .arg(m_lastSpeakerSectionsTableRefreshDurationMs)
            .arg(m_maxSpeakerSectionsTableRefreshDurationMs)
            .arg(m_lastSpeakerSectionsTableRowCount)
            .arg(m_lastSpeakerSectionsTableRefreshSkippedReason.isEmpty()
                     ? QStringLiteral("rebuilt")
                     : m_lastSpeakerSectionsTableRefreshSkippedReason)
