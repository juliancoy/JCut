...
void PreviewWindow::drawCompositedPreviewOverlay(QPainter* painter,
    ...
    if (m_interaction.viewMode == ViewMode::Audio || activeClips.isEmpty()) {
        ...
        drawPlaybackStatusOverlay(painter, safeRect);
        painter->restore();
        return;
    }
...
void PreviewWindow::drawCurrentSpeakerLabelOverlay(QPainter* painter, const QRect& compositeRect)
{
    ...
    const auto drawBlock = [&](const QVector<SpeakerOverlayLine>& blockLines, qreal verticalPosition) {
        ...
        const int centerY = static_cast<int>(std::round(qBound<qreal>(
            compositeRect.top() + (cardHeight * 0.5),
            compositeRect.top() + (compositeRect.height() * qBound<qreal>(0.0, verticalPosition, 1.0)),
            compositeRect.bottom() - (cardHeight * 0.5))));
    ...
    drawBlock(nameLines, m_interaction.currentSpeakerNameVerticalPosition);
    drawBlock(organizationLines, m_interaction.currentSpeakerOrganizationVerticalPosition);
    painter->restore();
}
