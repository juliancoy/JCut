...
1. FaceDetections exists (`keyframes` non-empty).
2. `enabled == true`.
...
No bootstrap/autofill accommodations from transcript FaceDetections on toggle.

## End-to-End Pathway

### 1. Generate tracking FaceDetections (speaker profile)
...
- `source_norm`: normalized coordinates in source media space `[0..1]`.
  - Examples: FaceDetections `x/y`, `box_size`, `box_left/top/right/bottom`.
...
`box_size` is treated as the canonical FaceDetections size (normalized to source min-side).
...
1. Tracking enablement is explicit (`enabled` + FaceDetections), never inferred from refs.
...
- Add full live `Size` retarget for existing keyframes using FaceDetections-aware re-solve.
