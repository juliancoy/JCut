    ...
    QString* errorMessage)
{
    ...
    if (!m_handoff->initialize(context, errorMessage)) {
        m_lastError = errorMessage ? *errorMessage : QStringLiteral("Failed to initialize Vulkan frame handoff.");
        m_handoff.reset();
        return false;
    }
    m_lastError.clear();
